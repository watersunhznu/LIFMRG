...
if __name__ == "__main__":
    label = torch.empty((16, 51), dtype=torch.long).random_(2)
    pre_tags = torch.randn((16, 51, 2), requires_grad=True)
    fl1 = focal_loss(alpha=0.01, gamma=2)
    ce = nn.CrossEntropyLoss()
    a1 = fl1(pre_tags.view(-1, 2), label.view(-1))
    b = ce(pre_tags.view(-1, 2), label.view(-1))
    display(a1, b)
