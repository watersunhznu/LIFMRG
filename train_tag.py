...
def script(args):
    ...
    evaluate(args, val_loader, encoderCNN, mlc, vocab, 0, '', tags_l)
    best_f1 = 0
    best_f1_epoch = 0
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (images, captions, prob, tags) in enumerate(train_loader):
            ...
            # loss = args.lambda_sent * loss_sent + args.lambda_word * loss_word + tags_loss * args.lambda_tag
            loss = tags_loss * args.lambda_tag
            ...
            if i % args.log_step == 0:
                print('train： Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}  >{:.5f}<'
                    .format(
                    epoch, args.num_epochs, i, total_step, loss.item(), tags_loss.item() * args.lambda_tag))
        ...
        f1_v = evaluate(args, val_loader, encoderCNN, mlc, vocab, epoch, title, tags_l)
        if best_f1 < f1_v:
            best_f1 = f1_v
            best_f1_epoch = epoch
    ...
    print(best_f1_epoch, best_f1)


def evaluate(args, val_loader, encoderCNN, mlc, vocab, epoch, title, tags_l):
    encoderCNN.eval()
    mlc.eval()
    # sentLSTM.eval()
    # wordLSTM.eval()
    ...
    for i, (images, captions, prob, tags) in enumerate(val_loader):
        ...
        if i % args.log_step == 0:
            print('val: Epoch [{}/{}], Step [{}/{}], Loss: {:.4f} >{:.5f}<'
                .format(
                epoch, args.num_epochs, i, total_step, loss.item(), tags_loss.item() * args.lambda_tag
            ))
    # assert len(references) == len(hypotheses)
    me = metrics.acc_f1(met_e)
    print('eval metrics', np.sum(met_e, axis=0), 'Acc Precision Recall F1-->>', me)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_tsv_path', type=str, default='train2.json', help='path of the training tsv file')
    parser.add_argument('--val_tsv_path', type=str, default='val2.json', help='path of the validating tsv file')
    ...
    parser.add_argument('--fl_alpha', type=float, default=0.3, help='')
    parser.add_argument('--fl_gamma', type=int, default=5, help='')
    ...
    parser.add_argument('--learning_rate_mlc', type=int, default=1e-4, help='learning rate for MLC Encoder')
